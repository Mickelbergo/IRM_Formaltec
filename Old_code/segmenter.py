...
class DecoderLinear(nn.Module):
    ...
    def forward(self, x, im_size):
        ...
        x = torch.reshape(x, (b, int(GS), int(hw/GS), c))
        x = x.permute((0,3,1,2)) # e(x, "b (h w) c -> b c h w", h=GS)
...
class MaskTransformer(nn.Module):
    def __init__(
        ...
        self.dec_proj = nn.Linear(d_encoder, d_model)
...
model_cfg = {
    ...
    "n_layers": 12, "distilled": False, "d_ff": 4*192, # d_model > 64
...
vit_model = Segmenter(encoder, decoder, n_cls=model_cfg["n_cls"])
...
# # change the keys, so that they mach
...
#     new_key = new_key.replace("decode_head", "decoder")
#     new_key = new_key.replace("patch_proj.weight", "proj_patch")
#     new_key = new_key.replace("classes_proj.weight", "proj_classes")
#     new_key = new_key.replace("mlp", "ffn")
    
#     new_state_dict[new_key] = value

# # remove the layers with different size weights
# new_state_dict.pop("decoder.cls_emb", None)
# new_state_dict.pop("decoder.mask_norm.weight", None)
# new_state_dict.pop("decoder.mask_norm.bias", None)
