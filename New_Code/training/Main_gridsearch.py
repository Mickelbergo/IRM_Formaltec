...
def train_once(train_config, preprocessing_config, train_ids, valid_ids, path, preprocessing_fn, detection_model, DEVICE,
    ...
    train_loader = DataLoader(train_dataset, batch_size=train_config["batch_size"], shuffle=False, 
                              num_workers=train_config["num_workers"], worker_init_fn=worker_init_fn, 
    ...
    train_epoch = TrainEpoch(model, CE_Loss, DICE_Loss, Focal_loss, lambdaa, segmentation, optimizer, device=DEVICE,
                             ...
                             scheduler=scheduler, mixed_prec=train_config["mixed_precision"])
    ...
    for epoch in range(train_config["num_epochs"]):
        ...
        if current_iou > max_score:
            ...
            model_filename = (f"best_model_{train_config['model_version']}_epoch{epoch}_"
